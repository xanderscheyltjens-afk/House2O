...
def absorbed_power_spectrum(absorption_wavelen, absorp_coeffs, sun_wavelen, irradiance, d, aoi, glass=False):
    ...
    absorbed_power_total = np.trapezoid(absorbed_power_density, common_wavelen)
...
def previous_main_file():
    ...
    LAT, LON = 51.22, 4.40
    TZ = "Europe/Brussels"
    DATETIME = "2024-06-27 15:00"    # local time (CEST = UTC+2, solar noon ≈ 13:30)
    ...
    if solar["sun_above_horizon"]:
        total_power = np.trapezoid(spectrum["poa_global"], spectrum["wavelength_nm"])
    ...
    DNI = np.trapezoid(spectrum["dni"], spectrum["wavelength_nm"])
    DHI = np.trapezoid(spectrum["dhi"], spectrum["wavelength_nm"])
    ...
    irradiance_ground_diffuse = np.maximum(irradiance_global-irradiance_direct-irradiance_sky_diffuse,0)
    total_power_cloudy = np.trapezoid(irradiance_global, spectrum["wavelength_nm"])
    ...
    aoi_glass = solar["aoi"]
    glass_filename = BASE_DIR + r"\Rubin-lowiron.csv"
    irradiance_direct, aot_direct, absorbed_power_total_glass_direct=air_glass_water(glass_filename=glass_filename, aoi_glass=aoi_glass, irradiance = irradiance_direct, sun_wavelen=sun_wavelen)
    ...
    irradiance_sky_diffuse, aot_sky, absorbed_power_total_glass_sky=air_glass_water(glass_filename=glass_filename, aoi_glass=aoi_sky_diffuse, irradiance = irradiance_sky_diffuse, sun_wavelen= sun_wavelen)
    # And finally ground diffuse
    aoi_ground_diffuse = 90-0.5788*surface_tilt+0.002693*surface_tilt**2 # Same paper as above
    irradiance_ground_diffuse, aot_ground, absorbed_power_total_glass_ground=air_glass_water(glass_filename=glass_filename, aoi_glass=aoi_ground_diffuse, irradiance = irradiance_ground_diffuse, sun_wavelen= sun_wavelen)
    ...
    # Oh well, we did the same with the glass absorption, it's not like anyone has a better idea
...
def general_use(LAT=51.222, LON=4.401, DATETIME="2024-06-27 15:00", surface_tilt=90, print_details=False): # The default date is VERY IMPORTANT ;)
    ...
    # Print some info if the user wants to check
    if print_details:
        if solar["sun_above_horizon"]:
            total_power = np.trapezoid(spectrum["poa_global"], spectrum["wavelength_nm"])
            print(f"  Solar zenith angle : {solar['apparent_zenith']:.1f}°")
    ...
    sun_wavelen = spectrum["wavelength_nm"]
    # Let's now adjust the direct and diffuse spectra from out clear sky model generated by SMARTS for cloudy conditions
    # We integrate the relevant spectra from SMARTS
    SMARTS_DNI = np.trapezoid(spectrum["dni"], spectrum["wavelength_nm"])
    SMARTS_DHI = np.trapezoid(spectrum["dhi"], spectrum["wavelength_nm"])
    # And then divide by data for cloudy days to get CMF
    ...
    irradiance_global = spectrum["poa_global"]*global_CMF
    #From the difference of global-direct-sky diffuse irradiances we find the remaining ground diffuse irradiance on a cloudy day
    irradiance_ground_diffuse = np.maximum(irradiance_global-irradiance_direct-irradiance_sky_diffuse,0) #prevents possible negative values due to errors
    total_power_cloudy = np.trapezoid(irradiance_global, spectrum["wavelength_nm"])
    if print_details:
        print(f"The total power with clouds is:{total_power_cloudy} W/m^2")
